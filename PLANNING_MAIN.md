...
- Raspberry Pi 3B
- 7" 480p Official RPi Display (touchscreen)
...
### Step 4: Configure Auto-Login & Display
...
cat << 'EOF' | sudo tee /home/pi/.xinitrc
#!/bin/bash
xset s off
xset -dpms
xset s noblank
exec openbox-session &
python3 /home/pi/tablepi/app/main.py
EOF
...
### Step 5: Configure Display Rotation (if needed)

```bash
# Edit boot config
sudo nano /boot/config.txt

# Add for rotation (0, 90, 180, 270)
display_rotate=0

# For 7" official display, ensure:
lcd_rotate=2  # If upside down
```

### Step 6: Configure Touchscreen

```bash
# The official 7" display should work out of the box
# If touch is inverted, add to /boot/config.txt:
lcd_rotate=2

# For rotation with touch, may need:
# dtoverlay=rpi-ft5406,touchscreen-inverted-x=1,touchscreen-swapped-x-y=1
```

### Step 7: Install TablePi Application
...
Type=simple
User=pi
WorkingDirectory=/home/pi/tablepi
Environment=DISPLAY=:0
Environment=PULSE_RUNTIME_PATH=/run/user/1000/pulse
ExecStart=/home/pi/tablepi/venv/bin/python app/main.py
Restart=always
RestartSec=5
...
```bash
# Install bluetooth and audio tools
...
# Pair manually first time (or use web UI after setup)
bluetoothctl
# > power on
# > agent on
# > scan on
...
### Step 10: Configure PulseAudio for Audio Device Switching
...
# The web UI will use pactl to switch between devices
```
